...
public class App implements Serializable {
     ...
     */
    public App(Collection collection, FileManager fileManager) {
     ...
     */
    public String  updateId(Integer id, MusicBand newMusicBand) {
        boolean flag = true;
        for (MusicBand musicBand : collection.getCollection()) {
            if (musicBand.getId().equals(id)) {
                collection.update(id, newMusicBand);
                flag = false;
            }
        }
        if (flag) {
            return ("Неправильно введен id, повторите команду:");
        }
        else {
